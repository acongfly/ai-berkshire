...
本 Codex skill 是 `skills/thesis-tracker.md` 的轻量包装层。`skills/thesis-tracker.md` 是 Claude Code 与 Codex 共同使用的单一事实来源。

## 执行要求

1. 在执行本 skill 时，先阅读仓库根目录的 `AGENTS.md`。
2. 完整阅读 `skills/thesis-tracker.md`，并按其中的流程、输出格式、数据纪律执行。
3. 用户在 Codex 中可显式输入 `$thesis-tracker` 触发；在 Claude Code 中继续使用 `/thesis-tracker`。
...
请打开并执行：`skills/thesis-tracker.md`
